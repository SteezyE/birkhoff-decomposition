/***************************************************************************************************
 *                                                                                                 *
 *  binary_bottleneck.c - implements binary search bottleneck decomposition algorithm              *
                   ...
                   double **_factors, int *perm_count)
{
    ...
    double fac_sum = 0.0, bottleneck_v;
    *perm_count = 0;
    while(1)
    {
        bottleneck_v = bottleneck_matching(col_ptrs, col_ids, col_vals, n, *k, best_col_to_row,
                                           best_row_to_col);
        ...
        // reuse column to row matching map for residual update
        int * update_pos = best_col_to_row;
        // store matching
        for(i=0; i<n; ++i)
        {
            int a = best_row_to_col[i];
            matchings[(*perm_count)*n+i] = a;
            for(j=col_ptrs[a]; j<col_ptrs[a+1]; ++j)
            {
                if(col_ids[j] == i)
                {
                    col_vals[j] -= bottleneck_v;
                    update_pos[i] = j;
                    break;
                }
            }
        ...
        // store factor
        factors[(*perm_count)++] = bottleneck_v;
        fac_sum += bottleneck_v;
        // check end condition
        double sos = 0.0;
        for(i=0; i<*k; ++i)
...
#if DEBUG_SMALL
        printf("perm_count: %d, best_factor: %lg, factor_sum: %f, "
               "frobenius_norm:  %f\n", *perm_count, bottleneck_v, fac_sum, sqrt(sos));
#endif
        ...
        for(i=0; i<n; ++i)
        {
            if(col_vals[update_pos[i]] <= ZERO_EPS)
            {
                ...
                update_pos[compact_sz++] = best_row_to_col[i];
            }
        }
    ...
    free(best_col_to_row); free(best_row_to_col);
#if DEBUG
